...
import { MapPin, ArrowRight, Building, CheckCircle, Clock, Droplets, Mountain, HardHat } from 'lucide-react';
    ...
    return (
        <div className="bg-white min-h-screen font-sans text-titan-navy relative">
            ...
            <section className="pt-48 pb-20 px-6 max-w-[1400px] mx-auto text-center relative z-10">
                ...
                >
                    <HardHat size={12} className="text-titan-red" />
                    Currently Active
                </motion.div>
                <motion.h1
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="text-5xl md:text-7xl font-black text-titan-navy mb-6 tracking-tighter"
                >
                ...
                >
                    Witness our ongoing commitment to building the future. These projects are currently under construction and shaping the skyline.
                </motion.p>
            </section>
            ...
            <section className="px-6 pb-32 max-w-[1400px] mx-auto">
                        ...
                        {filteredProjects.map((project) => (
                            <Link href={`/design-x/projects/${project.id}`} key={project.id} className="group block bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 flex flex-col h-full">
                                ...
                                <div className="aspect-[4/3] overflow-hidden relative">
                                    ...
                                    />
                                    {/* Status Badge */}
                                    <div className="absolute top-4 left-4 z-20 px-3 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wide flex items-center gap-1.5 shadow-sm bg-titan-navy text-white">
                                        <Clock size={10} />
                                        Under Construction
                                    </div>
                                </div>
                ...
                {filteredProjects.length === 0 && (
                    <div className="text-center py-20 bg-titan-bg rounded-2xl border border-dashed border-gray-300">
                        ...
                        <h3 className="text-xl font-bold text-titan-navy">No implementation projects found.</h3>
                        <p className="text-titan-navy-subtle">Check back soon for updates.</p>
                    </div>
                )}
            </section>
        </div>
