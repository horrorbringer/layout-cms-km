...
import { MapPin, Filter, ArrowRight, Building, Activity, CheckCircle, Clock, Droplets, Mountain } from 'lucide-react';
    ...
    return (
        <div className="bg-white min-h-screen font-sans text-titan-navy relative">
            ...
            <section className="pt-48 pb-20 px-6 max-w-[1400px] mx-auto text-center relative z-10">
                ...
                >
                    <Building size={12} className="text-titan-red" />
                    Our Portfolio
                </motion.div>
                <motion.h1
                ...
                >
                    OUR <span className="text-titan-red">PROJECTS</span>
                </motion.h1>
                <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                ...
                >
                    Delivering excellence across government, industrial, and private sectors. From complex water treatment plants to iconic high-rises.
                </motion.p>
            </section>
                ...
                <div className="max-w-[1400px] mx-auto flex flex-col items-center gap-6">
                    <div className="flex flex-wrap gap-4 justify-center">
                        ...
                        <div className="relative group">
                            ...
                            <Activity size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-titan-red pointer-events-none" />
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="px-6 pb-32 max-w-[1400px] mx-auto">
                <AnimatePresence mode='wait'>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredProjects.map((project) => (
                            <Link href={`/design-x/projects/${project.id}`} key={project.id} className="group block bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 flex flex-col h-full">
                                {/* Thumbnail */}
                                <div className="aspect-[4/3] overflow-hidden relative">
                                    ...
                                    />
                                    {/* Status Badge */}
                                    <div className={`absolute top-4 left-4 z-20 px-3 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wide flex items-center gap-1.5 shadow-sm ${project.status === 'Completed' ? 'bg-white text-green-700' : 'bg-titan-navy text-white'}`}>
                ...
                {filteredProjects.length === 0 && (
                    <div className="text-center py-20 bg-titan-bg rounded-2xl border border-dashed border-gray-300">
                        ...
                        <h3 className="text-xl font-bold text-titan-navy">No projects found.</h3>
                        <p className="text-titan-navy-subtle">Try adjusting your filters to see more results.</p>
                    </div>
                )}
            </section>
        </div>
