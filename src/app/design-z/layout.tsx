    ...
    const navItems = [
        {
            ...
            children: [
                { label: t('Company Profile'), href: '/design-z/about#profile', desc: t('Learn about our history') },
                { label: t('Leadership'), href: '/design-z/about#leadership', desc: t('Meet our team') },
                { label: t('Quality & Safety'), href: '/design-z/about#safety', desc: t('Our standards') }
            ]
        },
        {
            ...
            children: [
                {
                    ...
                    href: '/design-z/projects/completed',
        ...
        {
            label: t('News'), href: '/design-z/news',
            children: [
                { label: t('News & Updates'), href: '/design-z/news', desc: t('Latest announcements') },
                { label: t('Doc Collection'), href: '/design-z/documents', desc: t('Resources & documents') }
            ]
        },
        { label: t('Careers'), href: '/design-z/careers' },
        { label: t('Contact'), href: '/design-z/contact' }
    ];
    ...
    return (
        <div className={`bg-white text-titan-navy min-h-screen flex flex-col ${fontClassName}`}>
            ...
            <header className="fixed top-0 left-0 w-full z-[100]">
                ...
                {/* --- MAIN NAVBAR --- */}
                <nav className={`w-full transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-xl' : 'bg-transparent border-b border-white/10'}`}>
                    <div className="max-w-[1600px] mx-auto px-6">
                        <div className="flex justify-between items-center h-20">
                            {/* Logo Area */}
                            <Link href="/design-z" className="flex items-center gap-3 group cursor-pointer">
                                ...
                                <div className="leading-none flex flex-col justify-center">
                                    <span className={`block font-black text-2xl md:text-3xl tracking-tight transition-all duration-300 leading-none group-hover:text-accent-orange ${isScrolled ? 'text-titan-navy' : 'text-white'}`}>
                                        KIMMEX
                                    </span>
                                    <span className={`block text-[9px] font-bold tracking-[0.15em] uppercase transition-colors duration-300 mt-0.5 ${isScrolled ? 'text-titan-navy/50' : 'text-white/60'}`}>
                                ...
                                {navItems.map((item, i) => (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {item.children && (
                                                <ChevronDown strokeWidth={2.5} size={12} className={`transition-transform duration-300 group-hover/nav:-rotate-180 group-hover/nav:text-accent-orange ${isScrolled ? 'text-titan-navy/50' : 'text-white/50'}`} />
                                            )}
                                            {/* Bottom indicator */}
                                        ...
                                        {item.children && (
                                            <div className="absolute top-full left-0 pt-0 opacity-0 invisible group-hover/nav:opacity-100 group-hover/nav:visible transition-all duration-300 transform translate-y-2 group-hover/nav:translate-y-0 z-50">
                                                <div className="bg-titan-navy/95 backdrop-blur-xl shadow-[0_40px_80px_-12px_rgba(0,0,0,0.5)] rounded-2xl border border-white/10 min-w-[280px] p-2">
                                                    ...
                                                    <div className="">
                                                        {item.children.map((child, idx) => (
                                                            <div key={idx} className="relative group/nested">
                                                                <Link
                                                                    href={child.href}
                                                                    className="flex items-center justify-between px-4 py-3.5 rounded-xl hover:bg-white/10 transition-all duration-200 group/item"
                                                                >
                                                                    <div>
                                                                        <div className={`font-medium text-white/90 group-hover/item:text-accent-orange transition-colors ${textDropdown}`}>
                                                                            {child.label}
                                                                        </div>
                                                                        {child.desc && (
                                                                            <div className="text-[10px] text-white/50 mt-0.5 group-hover/item:text-white/70 transition-colors">
                                                                                {child.desc}
                                                                            </div>
                                                                        )}
                                                                    </div>
                                                                    {(child as any).children && (
                                                                        <ArrowRight size={14} className="text-white/30 group-hover/item:text-accent-orange transition-colors" />
                                                                    )}
                                                                </Link>
                                                                ...
                                                                {(child as any).children && (
                                                                    <div className="absolute left-full top-0 ml-2 opacity-0 invisible group-hover/nested:opacity-100 group-hover/nested:visible transition-all duration-300 transform translate-x-2 group-hover/nested:translate-x-0 z-[60]">
                                                                        <div className="bg-titan-navy/95 backdrop-blur-xl shadow-[0_40px_80px_-12px_rgba(0,0,0,0.5)] rounded-2xl overflow-hidden border border-white/10 min-w-[240px] p-2">
                                                                            ...
                                                                            <div className="">
                                                                                {(child as any).children.map((sub: any, subIdx: number) => (
                                                                                    <Link
                                                                                        key={subIdx}
                                                                                        href={sub.href}
                                                                                        className={`group/sub flex items-center justify-between px-4 py-3 rounded-xl ${textDropdown} font-medium text-white/70 hover:text-white hover:bg-white/10 transition-all`}
                                                                                    >
                                                                                        <span>{sub.label}</span>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className={`w-10 h-10 rounded-lg flex items-center justify-center hover:bg-accent-orange hover:text-white transition-all ${isScrolled ? 'bg-gray-100 text-titan-navy' : 'bg-white/10 text-white hover:bg-white/20'}`}
                                >
                                    <Search size={18} />
                                </button>
                                ...
                                <button
                                    ...
                                    className={`lg:hidden w-10 h-10 rounded-lg flex items-center justify-center ${isScrolled ? 'bg-titan-navy text-white' : 'bg-white/10 text-white'}`}
                                >
                    ...
                    {/* Bottom Border */}
                    <div className={`h-[1px] ${isScrolled ? 'bg-gray-100' : 'bg-transparent'}`}></div>
                </nav>
                        ...
                        >
                            ...
                            <div className="p-5">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        { label: t('Projects'), href: '/design-z/projects', icon: '🏗️' },
                                        { label: t('Services'), href: '/design-z/services', icon: '⚙️' },
                                        { label: t('About Us'), href: '/design-z/about', icon: '🏢' },
                                        { label: t('Contact'), href: '/design-z/contact', icon: '📞' },
                                    ].map((link, i) => (
                                        <Link
                                            key={i}
                                            href={link.href}
