...
const categories = ['All', 'Trending', 'CORPORATE', 'ENVIRONMENT', 'CSR', 'INDUSTRY', 'ANNOUNCEMENT', 'Updates', 'Awards', 'Safety', 'Sustainability', 'Culture', 'Community', 'Innovation', 'Systems'];
const years = ['All', '2026', '2025', '2024'];

const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
        'Awards': 'bg-amber-50 text-amber-600 border-amber-100',
        'Updates': 'bg-blue-50 text-blue-600 border-blue-100',
        ...
        'ANNOUNCEMENT': 'bg-indigo-50 text-indigo-600 border-indigo-100',
        'INDUSTRY': 'bg-zinc-50 text-zinc-600 border-zinc-100',
        'CSR': 'bg-pink-50 text-pink-600 border-pink-100',
        'ENVIRONMENT': 'bg-lime-50 text-lime-600 border-lime-100',
        'CORPORATE': 'bg-sky-50 text-sky-600 border-sky-100',
        'Community': 'bg-teal-50 text-teal-600 border-teal-100',
    };
    return colors[category] || 'bg-gray-50 text-gray-600 border-gray-100';
};

// Rename to NewsPageView and accept props
export default function NewsPageView({ initialNews }: { initialNews: any[] }) {
    const { t, language } = useLanguage();

    // Use passed news or fallback to static import
    const allNewsItems = initialNews || allNews;

    const trendingNews = allNewsItems.filter(n => n.trending || n.isTrending);
    ...
    // Filtered News Items
    const filteredNews = allNewsItems.filter((n: any) => {
        const matchesCategory = activeCategory === 'All' ||
            (activeCategory === 'Trending' ? (n.trending || n.isTrending) :
                (n.category === activeCategory ||
                    (activeCategory === 'Updates' && n.category === 'Project Updates') ||
                    (activeCategory === 'Project Updates' && n.category === 'Updates')));
        const matchesYear = activeYear === 'All' || n.year === activeYear;
        return matchesCategory && matchesYear;
    });

    const activeFeaturedNews = filteredNews.length > 0 ? filteredNews[0] : null;
    const gridNews = activeFeaturedNews
        ? filteredNews.filter((n: any) => n.id !== activeFeaturedNews.id)
        : [];
    ...
    return (
        <div className="bg-gray-50/50 min-h-screen text-titan-navy pb-24">
            ...
            <div className="max-w-[1600px] mx-auto px-4 md:px-6 pt-10">
                ...
                {/* --- HERO: MAGAZINE COVER --- */}
                {!activeFeaturedNews ? (
                    <div className="mb-16 py-24 text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
