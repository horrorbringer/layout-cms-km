...
import { ArrowLeft, MapPin, Activity, HelpCircle, ArrowRight, Share2, Maximize, User, Calendar, CheckCircle2, AlertTriangle, Shield } from 'lucide-react';
import Link from 'next/link';
import { motion } from 'framer-motion';
import Image from 'next/image';
import { useLanguage, getLocalizedText } from '../../../context/LanguageContext';

export default function ProjectDetailView({ project, relatedProjects }: { project: any; relatedProjects: any[] }) {
    const { t, language } = useLanguage();

    const backLink = project.status?.en === 'Completed' || project.status === 'Completed'
        ? '/design-z/projects/completed'
        : '/design-z/projects/implementation';

    const backLabel = project.status?.en === 'Completed' || project.status === 'Completed'
                        ...
                        <div className="mb-16">
                            ...
                            <div className="space-y-8 text-lg text-titan-navy-subtle leading-relaxed">
                                <div>
                                    ...
                                    <div
                                        className="prose prose-sm xl:prose-base max-w-none text-titan-navy-subtle"
                                        dangerouslySetInnerHTML={{ __html: getLocalizedText(project.description?.background || { en: project.description?.en || project.description || '' }, language) }}
                                    />
                                </div>
                                {project.description?.objectives && (
                                    <div>
                                        ...
                                        <div
                                            className="prose prose-sm xl:prose-base max-w-none text-titan-navy-subtle"
                                            dangerouslySetInnerHTML={{ __html: getLocalizedText(project.description.objectives, language) }}
                                        />
                                    </div>
                                )}
                                {project.description?.concept && (
                                    <div>
                                        ...
                                        <div
                                            className="prose prose-sm xl:prose-base max-w-none text-titan-navy-subtle"
                                            dangerouslySetInnerHTML={{ __html: getLocalizedText(project.description.concept, language) }}
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="group">
                                    ...
                                    <div className="flex items-center gap-3 font-bold text-titan-navy text-lg">
                                        ...
                                        {project.year} <span className={`text-xs px-2 py-1 rounded text-white ${project.status?.en === 'Completed' || project.status === 'Completed' ? 'bg-green-600' : 'bg-orange-500'}`}>{getLocalizedText(project.status, language)}</span>
                                    </div>
                                </div>
                            </div>
