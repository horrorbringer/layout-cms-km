...
export default function CareerDetailPage() {
    ...
    const job = jobData.find(j => j.id === idStr) || jobData[0];
                    ...
                    <div className="lg:col-span-7 space-y-12">
                        ...
                        <div className="prose prose-lg text-titan-navy-subtle max-w-none">
                            <h3 className="text-xl font-black text-titan-navy mb-4 flex items-center gap-2">
                                <Sparkles size={20} className="text-titan-red" /> {t('Role Overview')}
                            </h3>
                        ...
                        <div>
                            <h3 className="text-xl font-black text-titan-navy mb-6 flex items-center gap-2 pb-4 border-b border-gray-100">
                                <Briefcase size={20} className="text-titan-red" /> {t('Key Responsibilities')}
                            </h3>
                        ...
                        <div>
                            <h3 className="text-xl font-black text-titan-navy mb-6 flex items-center gap-2 pb-4 border-b border-gray-100">
                                <UserCheck size={20} className="text-titan-red" /> {t('Qualifications')}
                            </h3>
                    ...
                    <div className="lg:col-span-5">
                        <div className="sticky top-24 space-y-6" id="application-form">
                            ...
                            <div className="bg-white border border-gray-200 shadow-2xl rounded-2xl p-8 relative overflow-hidden">
                                ...
                                <div className="mb-8">
                                    <h3 className="text-2xl font-black text-titan-navy mb-2">{t('Apply Now')}</h3>
                                    <p className="text-titan-navy-subtle text-sm">{t('Join us as a')} <span className="font-bold text-titan-navy">{getLocalizedText(job.title, language)}</span>.</p>
                                </div>
                                ...
                                <form onSubmit={(e) => { e.preventDefault(); alert('Application Submitted (Mock)'); }}>
                                    <div className="space-y-5 mb-8">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm text-titan-navy focus:border-titan-red focus:ring-1 focus:ring-titan-red focus:outline-none transition-all" placeholder={language === 'kh' ? 'ឧ. សុខ' : 'John'} required />
                                            </div>
                                            <div>
                                                <label className="block text-[10px] font-bold uppercase tracking-widest text-titan-navy mb-1.5">{t('Last Name')}</label>
                                                <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm text-titan-navy focus:border-titan-red focus:ring-1 focus:ring-titan-red focus:outline-none transition-all" placeholder={language === 'kh' ? 'ឧ. ដារ៉ា' : 'Doe'} required />
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-bold uppercase tracking-widest text-titan-navy mb-1.5">{t('Phone Number required')}</label>
                                        ...
                                        <div>
                                            ...
                                            <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:bg-gray-50 hover:border-titan-red/50 transition-colors cursor-pointer relative group">
                                                ...
                                                <p className="text-xs font-bold text-titan-navy group-hover:text-titan-red transition-colors">
                                                    {formData.cv ? formData.cv.name : t('Click to Upload or Drag & Drop')}
                                                </p>
                                    ...
                                    <button className="w-full bg-titan-navy text-white font-bold uppercase tracking-widest py-4 rounded-lg hover:bg-titan-red transition-all shadow-lg flex items-center justify-center gap-2 text-xs group">
                                        {t('Submit Application')} <Send size={14} className="group-hover:translate-x-1 transition-transform" />
                                    </button>
                                </form>
                            </div>
