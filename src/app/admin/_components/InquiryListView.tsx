...
export interface ContactMessage {
    ...
    message: string;
    date: string;
    status: 'NEW' | 'READ' | 'REPLIED' | 'CONTACTED' | 'QUALIFIED' | 'CLOSED';
}
...
export default function InquiryListView({
    ...
    const handleDelete = async (id: string) => {
        const isConfirmed = await confirm({
            ...
            message: 'Are you sure you want to delete this contact inquiry? This record will be permanently removed.',
            confirmText: 'Delete',
            type: 'danger'
        });
        ...
        if (isConfirmed) {
            try {
                const res = await fetch('/api/cms/inquiries/delete', {
                    method: 'POST',
    ...
    const handleUpdateStatus = async (id: string, status: ContactMessage['status']) => {
        try {
            const res = await fetch('/api/cms/inquiries/status', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
